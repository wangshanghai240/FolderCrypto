...
# 卸载时清理的右键菜单注册表键（ContextMenuRegistrar 写入到 HKCU\Software\Classes\*\shell 等）。
# 这些键并非 MSI 安装表组成部分（由 FolderCrypto.Shell install 运行时写入），
# 因此必须在 MSI 卸载时用 RemoveRegistryKey 显式删除，否则卸载后右键菜单残留。
$contextMenuKeys = @(
